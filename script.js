...
shufflepuzzle.addEventListener('click', e => {
  const localPadding = 20; // Reduce padding to give more space
  const localLeft = localPadding;
  const localRight = canvas.width - board.pw - localPadding;
  const localTop = localPadding;
  const localBottom = canvas.height - board.ph - localPadding;
  ...
  for (let i = 0; i < board.pieces.length; i++) {
    ...
    // Constrain the random positions within the visible canvas
    randomPosition.push({
      x: Math.max(localLeft, Math.min(localRight, localLeft - localX + Math.random() * (localRight - localLeft))),
      y: Math.max(localTop, Math.min(localBottom, localTop - localY + Math.random() * (localBottom - localTop)))
    });
...
const reset = () => {
  ...
  image.addEventListener('load', e => {
    ...
    // Set desired dimensions to 50% of the screen size
    let desiredWidth = screenWidth * 0.99;
    let desiredHeight = screenHeight * 0.99;
...
canvas.width = image.width + 2 * OFFSET;
canvas.height = image.height + 200; // Add extra height for moving pieces

// Adjust offsets if needed
const extraOffsetX = OFFSET;
const extraOffsetY = OFFSET;

board = new Board(ROWS, COLS, image, extraOffsetX, extraOffsetY, RADIUS);
